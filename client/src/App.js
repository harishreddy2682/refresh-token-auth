...
function App() {
  ...
  const [logout, logoutSuccess] = useState(false);
  ...
  const handleLogout = async () => {
    await axiosJWT.post(`http://localhost:5000/api/logout/`, { token: user.refreshToken }, {
      headers: { authorization: `Bearer ${user.accessToken}` }
    })
      ...
      {user ? (
        ...
        <div className="login">
          <form onSubmit={handleSubmit}>
            <span className="formTitle">Auth with JWT Refresh tokens</span>
            <input
              type="text"
